...
REM Start HTTP server in background (hidden window) from project root
start /B /MIN "" python -m http.server 8000 --directory .. >nul 2>&1
...
REM Check if update_server.py exists in backend folder
if not exist "..\backend\update_server.py" (
    echo [%DATE% %TIME%] ERROR: backend\update_server.py not found! >> %LOGFILE%
    goto :END
)

REM Start Update server in background (hidden window) from backend folder
start /B /MIN "" python ..\backend\update_server.py >nul 2>&1

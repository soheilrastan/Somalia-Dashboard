...
def run_update_script():
    ...
    try:
        ...
        # Run the update script (in tools folder)
        script_path = os.path.join(os.path.dirname(__file__), '..', 'tools', 'update_osm_roads.py')
        ...
        if process.returncode == 0:
            ...
            output_dir = Path(os.path.dirname(__file__)) / '..' / 'data_warehouse' / 'roads' / 'roads_by_region_latest'
...
def run_download_with_progress(version_date, resource_url, format_type):
    ...
    try:
        ...
        output_dir = base_dir / 'data_warehouse' / 'roads' / f'roads_by_region_{folder_date}'
        temp_dir = base_dir / 'temp_roads_download'
        ...
        boundaries_path = base_dir / 'layers' / 'boundaries' / 'adm1' / 'somalia_adm1_boundaries.geojson'
...
def uninstall_roads():
    ...
    try:
        ...
        folder_name = data.get('folder', '')  # New: accept direct folder name

        # Determine folder based on version key or direct folder name
        if version == '2024':
            folder = 'roads_by_region_2024_07_23'
        elif version == 'latest' or version == '2026':
            folder = 'roads_by_region_latest'
        elif folder_name and folder_name.startswith('roads_by_region_'):
            # Accept direct folder name (for custom versions)
            folder = folder_name
        elif version.startswith('roads_by_region_'):
            # Version key is actually the folder name
            folder = version
        else:
            return jsonify({'error': 'Invalid version. Use "2024", "latest", or provide folder name'}), 400

        # Roads are in data_warehouse/roads/
        folder_path = os.path.join(os.path.dirname(__file__), '..', 'data_warehouse', 'roads', folder)
